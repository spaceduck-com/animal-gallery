...
export const fetchAnimalsV1 = async () => {
    await sleep(150);
    const animals = ALL_ANIMALS.slice(0, ITEMS_PER_PAGE);
    return { animals };
}

export const fetchAnimalsV2 = async ({ page = 0 } : { page?: number }) => {
    await sleep(150);
    const offset = page * ITEMS_PER_PAGE;
    const animals = ALL_ANIMALS.slice(offset, offset + ITEMS_PER_PAGE);
    const count = ALL_ANIMALS.length;
    const pageCount = Math.ceil(count / ITEMS_PER_PAGE);
    return { animals, count , pageCount };
}